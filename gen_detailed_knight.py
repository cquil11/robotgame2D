#!/usr/bin/env python3
"""Generate highly detailed knight sprites at 20x30"""
...
def draw_detailed_knight(image, x, y, direction='right', pose='idle', frame=0):
    ...
    if pose == 'idle':
        ...
        if not flip:
            ...
            # Mirrored
            draw.ellipse([cx-7, cy-5, cx-4, cy+2], fill=armor_mid, outline=armor_dark)
            draw.ellipse([cx-6, cy-4, cx-5, cy+1], fill=armor_bright)
            draw.ellipse([cx+4, cy-5, cx+7, cy+2], fill=armor_mid, outline=armor_dark)
            draw.ellipse([cx+5, cy-4, cx+6, cy+1], fill=armor_bright)
        ...
        if not flip:
            ...
            draw.ellipse([cx+5, cy+8, cx+7, cy+12], fill=armor_mid)
            ...
            draw.polygon([(cx-7, cy-14), (cx-9, cy-12), (cx-8, cy+5)], fill=sword_bright)
            draw.polygon([(cx-8, cy-14), (cx-9, cy-12), (cx-8, cy+5)], fill=sword_dark)
            ...
            draw.ellipse([(cx-8, cy+4), (cx-6, cy+6)], fill=armor_bright)
...
for direction in ['left', 'right']:
    # Base poses
    for pose_name, pose in [('idle', 'idle'), ('jump', 'jump'), ('hit', 'hit')]:
        img = Image.new('RGBA', (20, 30), (0, 0, 0, 0))
        draw_detailed_knight(img, 0, 0, direction=direction, pose=pose)
        img.save(f'images/player_{direction}.png' if pose_name == 'idle' else f'images/player_{pose_name}_{direction}.png')
        print(f'Generated player_{pose_name}_{direction}.png')
    ...
    for attack in ['normal', 'critical', 'heavy']:
        ...
        for frame in range(frames):
            ...
            img.save(f'images/player_attack_{attack}_{direction}_{frame}.png')
            print(f'Generated player_attack_{attack}_{direction}_{frame}.png')

print('[OK] All detailed knight sprites generated at 20x30')
