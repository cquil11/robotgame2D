...
SCALE = 2
HR_W, HR_H = 60, 90  # Draw at 3x size, then downscale to 20x30
...
def draw_knight_base(facing_right=True, swing=False):
    ...
    offset_x = 15 if facing_right else HR_W - 15 - 12
    offset_y = 15

    # Legs
    add_shade_rect(d, [offset_x, offset_y + 15, offset_x + 12, offset_y + 22], CLOTH, highlight=CLOTH_DARK)
    add_shade_rect(d, [offset_x, offset_y + 22, offset_x + 12, offset_y + 25], LEATHER, shadow=(90, 50, 30, 255))

    # Torso
    add_shade_rect(d, [offset_x, offset_y + 5, offset_x + 12, offset_y + 15], ARMOR, highlight=ARMOR_LIGHT, shadow=ARMOR_DARK)
    add_shade_rect(d, [offset_x + 2, offset_y + 7, offset_x + 10, offset_y + 13], ARMOR_DARK, highlight=ARMOR_LIGHT)

    # Belt
    add_shade_rect(d, [offset_x, offset_y + 13, offset_x + 12, offset_y + 15], LEATHER, shadow=(80, 50, 25, 255))

    # Shoulders
    add_shade_rect(d, [offset_x - 2, offset_y + 3, offset_x + 14, offset_y + 6], ARMOR_DARK)
    add_shade_rect(d, [offset_x - 2, offset_y + 6, offset_x + 14, offset_y + 8], CLOTH, shadow=CLOTH_DARK)

    # Arms
    arm_y = offset_y + 6
    arm_len = 10 if swing else 7
    arm_x = offset_x + (12 if facing_right else -arm_len)
    add_shade_rect(d, [arm_x, arm_y, arm_x + arm_len, arm_y + 3], CLOTH, shadow=CLOTH_DARK)
    add_shade_rect(d, [arm_x, arm_y + 3, arm_x + arm_len, arm_y + 4], ARMOR, highlight=ARMOR_LIGHT)
    ...
    d.rectangle([head_x, head_y + 3, head_x + head_w, head_y + 4], fill=VISOR)
    d.rectangle([head_x + 3, head_y + 4, head_x + 9, head_y + 6], fill=SKIN)
    d.rectangle([head_x + 5, head_y - 1, head_x + 7, head_y + 1], fill=CLOTH)

    # Cape
    cape_x = offset_x + (10 if facing_right else -2)
    d.rectangle([cape_x, offset_y + 6, cape_x + 2, offset_y + 22], fill=(150, 20, 20, 140))
    ...
    guard_w = 3
    if facing_right:
        d.rectangle([sx1 - 1, sy1 + 1, sx1 + guard_w, sy1 + 3], fill=STEEL_DARK, outline=OUTLINE)
    else:
        d.rectangle([sx1 - guard_w, sy1 + 1, sx1 + 1, sy1 + 3], fill=STEEL_DARK, outline=OUTLINE)

    if swing:
        glow_x = sx2
        d.ellipse([glow_x - 2, sy2 - 2, glow_x + 3, sy2 + 3], fill=GLOW)
...
# Base sprites (20x30) - draw at 3x then downscale
for facing_right in (True, False):
    ...
    img = draw_knight_base(facing_right, False)
    img = img.transform((HR_W, HR_H), Image.AFFINE, (1, 0, 0, 0, 1, -3))
    final = downscale(img, 20, 30)
...
for frame in range(3):
    ...
    slash_len = 12 + frame * 6
    alpha = 230 - frame * 60
    d.line([32, 18, 32 + slash_len, 15], fill=(255, 240, 160, alpha), width=1)
    d.line([32, 18, 32 + slash_len, 21], fill=(255, 240, 160, alpha), width=1)
    glow = Image.new('RGBA', knight.size, (0, 0, 0, 0))
    gd = ImageDraw.Draw(glow)
    gd.line([32, 18, 32 + slash_len, 18], fill=(255, 240, 160, alpha//2), width=2)
...
for frame in range(4):
    ...
    for thick in range(2):
        d.arc([32 - 18 + thick, 15 - 18 + thick, 32 + 18 + thick, 15 + 18 + thick], start, end, fill=(255, 170, 100, 240 - frame * 45), width=1)
    if frame == 3:
        for i in range(3):
            d.ellipse([38 - i, 22 - i, 42 + i, 26 + i], outline=(255, 200, 110, 220 - i*30), width=1)
    glow = Image.new('RGBA', knight.size, (0, 0, 0, 0))
    gd = ImageDraw.Draw(glow)
    gd.arc([32 - 18, 15 - 18, 32 + 18, 15 + 18], start, end, fill=(255, 120, 60, 120), width=2)
    glow = glow.filter(ImageFilter.GaussianBlur(1))
    knight = Image.alpha_composite(knight, glow)
...
print('[OK] Player sprites with better detail-to-size ratio (20x30 final size)')
